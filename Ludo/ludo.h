...
struct Player {
	...
	Player(int startX, int startY, int nextX, int nextY)
		: startX(startX), startY(startY), currentX(startX), currentY(startY), initXDir(nextX), initYDir(nextY), xDir(nextX), yDir(nextY) {}
...
void move(std::list<Square>::iterator& posInList, Player& player, std::list<Square>::iterator end, int dice, int turn) {
